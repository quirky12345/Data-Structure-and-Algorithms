...
//Below function accepts sttring and a pointer variable as an argument
...
Node* constructtree(string s,int *start){
    ...
    //As soon as we see first right parenthesis from the current node
    //we start to construct the tree in the left
    if(*start<s.size() && s[*start] == '('){
    	*start = *start + 1;
    	root->left = constructtree(s,start);
    }
    if(*start<s.size() && s[*start] == ')')*start = *start + 1;

    //As soon as we see second right parenthesis from the current node
